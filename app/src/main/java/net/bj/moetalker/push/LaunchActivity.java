...
public class LaunchActivity extends Activity {
    ...
    /**
     * 给背景设置一个动画
     * @param endProgsess 动画的结束进度
     * @param endCallback 动画结束时触发
     */
    private void startAnim(float endProgsess, final Runnable endCallback){
        //获取一个最终的颜色，等同于
        //UiCompat.getColor(getResources(),R.color.white);
        int finalColor = Resource.Color.WHITE;
        //运算当前进度的颜色
        ArgbEvaluator evaluator = new ArgbEvaluator();
        int endColor = (int) evaluator.evaluate(endProgsess,mBgDrawable.getColor(),finalColor);
        //构建一个属性动画
